...
class LOXSolverConfig:
    ...
    deformable_recycle_cr: bool = True
    """Whether to augment CR with recent candidate-solve corrections.

    The corrections are A-orthogonalized and re-projected against the current
    right-hand side before each restarted CR solve. This recycles a small
    correction subspace across LOX iterations while preserving the fixed CR
    iteration count. Enabled by default.
    """
    ...
    deformable_preconditioner: Literal["incomplete_ldlt", "multilevel", "two_level", "block_jacobi"] = "two_level"
    ...
    ``"multilevel"`` adds a component-global correction above the local coarse
    blocks. ``"incomplete_ldlt"`` provides a global triangular correction at
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.deformable_recycle_cr, bool):
            raise ValueError(f"Invalid deformable_recycle_cr: {self.deformable_recycle_cr}. Must be a boolean.")
        ...
        if self.deformable_preconditioner not in ("incomplete_ldlt", "multilevel", "two_level", "block_jacobi"):
            raise ValueError(
                "Invalid deformable_preconditioner: "
                f"{self.deformable_preconditioner!r}. Must be 'incomplete_ldlt', 'multilevel', "
                "'two_level', or 'block_jacobi'."
            )
