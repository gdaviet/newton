...
from .deformable_two_level import DeformableMultilevel, DeformableTwoLevel
...
class _RecyclingCR(wpl.CR):
    """Fixed-count CR augmented by recent candidate-solve corrections."""

    def __init__(
        self,
        *args,
        previous_velocity: wp.array,
        previous_matrix_velocity: wp.array,
        recycled_basis: wp.array,
        recycled_matrix_basis: wp.array,
        recycled_basis_denominator: wp.array,
        recycled_basis_valid: wp.array,
        recycling_started: wp.array,
        replacement_cursor: wp.array,
        reductions: wp.array,
        ...
        **kwargs,
        ...
        self._previous_matrix_velocity = previous_matrix_velocity
        self._recycled_basis = recycled_basis
        self._recycled_matrix_basis = recycled_matrix_basis
        self._recycled_basis_denominator = recycled_basis_denominator
        self._recycled_basis_valid = recycled_basis_valid
        self._recycling_started = recycling_started
        self._replacement_cursor = replacement_cursor
    ...
    def _run(self, A, b, x, M):
        # This is Warp's fixed-count preconditioned recurrence with one
        # correction-space projection after the true initial residual is built.
        assert M is not None
        batch_offsets = A.batch_offsets
        tiled_dot = self._tiled_dot
        r, z = self._r_and_z_buf[0], self._r_and_z_buf[1]
        r_copy, az = self._r_and_Az[0], self._r_and_Az[1]
        y, ap = self._y_and_Ap_buf[0], self._y_and_Ap_buf[1]
        p = self._p
        r_norm_sq = tiled_dot.col(0)
        z_az_new = tiled_dot.col(1)
        z_az_old, atol_sq = self._residuals[0], self._residuals[1]

        _wpl_internal._initialize_absolute_tolerance(b, self._tol, self._atol, tiled_dot, atol_sq)
        A.matvec(x, b, r, alpha=-1.0, beta=1.0)
        self._recycled_reductions.zero_()
        wp.launch(
            _reduce_recycled_cr_basis,
            dim=x.shape[0],
            inputs=[
                x,
                r,
                ...
                self._recycled_basis,
                self._recycled_basis_valid,
                self._recycled_projection,
                self._replacement_cursor,
            ...
            outputs=[self._recycled_reductions],
            device=self._device,
        )
        wp.launch(
            _compute_recycled_cr_coefficients,
            dim=batch_offsets.shape[0] - 1,
            inputs=[
                self._recycled_reductions,
        ...
        def update_residual_direction_products():
            A.matvec(z, az, az, alpha=1.0, beta=0.0)
            r_copy.assign(r)
            tiled_dot.compute(self._r_and_z_buf, self._r_and_Az)

        z.zero_()
        M.matvec(r, z, z, alpha=1.0, beta=0.0)
        update_residual_direction_products()
        p.assign(z)
        ap.assign(az)

        for iteration in range(self._maxiter):
            z_az_old.assign(z_az_new)
            ...
            wp.launch(
                ...
                inputs=[
                    ...
                    y,
                    batch_offsets,
                    self._dofs_per_entry,
                ],
            )
            if iteration + 1 < self._maxiter:
                update_residual_direction_products()
                wp.launch(
        ...
        # Refresh the final residual norm without the unused next A*z product.
        r_copy.assign(r)
        tiled_dot.compute(self._r_and_z_buf, self._r_and_Az)
        return self._cur_iter_and_condition[0:1], r_norm_sq, atol_sq
...
def _reduce_recycled_cr_basis(
    ...
    basis: wp.array2d[wp.vec3],
    basis_valid: wp.array[wp.int32],
    previous_projection: wp.array2d[wp.float32],
    replacement_cursor: wp.array[wp.int32],
    ...
    particle = wp.tid()
    if recycling_started[0] != 0 and packed_iterative[particle] != 0 and world_active[packed_world[particle]] != 0:
        ...
        raw_direction = velocity[particle] - previous_velocity[particle]
        raw_matrix_direction = current_matrix_velocity - previous_matrix_velocity[particle]
        wp.atomic_add(reductions, 0, component, wp.dot(raw_direction, raw_matrix_direction))
        wp.atomic_add(reductions, 1, component, wp.dot(raw_direction, particle_residual))
        replacement_slot = replacement_cursor[0]
        previous_correction = wp.vec3(0.0)
        for slot in range(_RECYCLED_CR_DIRECTION_COUNT):
            if basis_valid[slot] != 0:
                old_direction = basis[slot, particle]
                previous_correction += previous_projection[slot, component] * old_direction
                if slot != replacement_slot:
                    wp.atomic_add(
                        reductions,
                        3 + slot,
                        component,
                        wp.dot(old_direction, raw_matrix_direction),
                    )
                    wp.atomic_add(
                        reductions,
                        3 + _RECYCLED_CR_DIRECTION_COUNT + slot,
                        component,
                        wp.dot(old_direction, particle_residual),
                    )
...
def _compute_recycled_cr_coefficients(
    ...
    component = wp.tid()
    replacement_slot = replacement_cursor[0]
    raw_energy = reductions[0, component]
    new_denominator = raw_energy
    new_numerator = reductions[1, component]
    for slot in range(_RECYCLED_CR_DIRECTION_COUNT):
        orthogonalization[slot, component] = 0.0
        projection[slot, component] = 0.0
        denominator = basis_denominator[slot, component]
        if slot != replacement_slot and basis_valid[slot] != 0 and denominator > 1.0e-30:
            coupling = reductions[3 + slot, component]
            old_residual_dot = reductions[3 + _RECYCLED_CR_DIRECTION_COUNT + slot, component]
            coefficient = coupling / denominator
            orthogonalization[slot, component] = coefficient
            projection[slot, component] = old_residual_dot / denominator
            new_denominator -= coupling * coefficient
            new_numerator -= coefficient * old_residual_dot

    minimum_denominator = wp.max(1.0e-6 * wp.abs(raw_energy), 1.0e-30)
    basis_denominator[replacement_slot, component] = 0.0
    ...
    if restart:
        ...
        if wp.isfinite(coefficient):
            basis_denominator[replacement_slot, component] = new_denominator
            projection[replacement_slot, component] = coefficient


@wp.kernel
def _apply_recycled_cr_basis(
    right_hand_side: wp.array[wp.vec3],
    basis: wp.array2d[wp.vec3],
    matrix_basis: wp.array2d[wp.vec3],
    basis_denominator: wp.array2d[wp.float32],
    basis_valid: wp.array[wp.int32],
    replacement_cursor: wp.array[wp.int32],
    orthogonalization: wp.array2d[wp.float32],
    ...
    if packed_iterative[particle] != 0 and world_active[packed_world[particle]] != 0:
        ...
        previous_matrix_velocity[particle] = current_matrix_velocity
        if recycling_started[0] != 0:
            replacement_slot = replacement_cursor[0]
            new_direction = raw_direction
            new_matrix_direction = raw_matrix_direction
            correction = wp.vec3(0.0)
            matrix_correction = wp.vec3(0.0)
            for slot in range(_RECYCLED_CR_DIRECTION_COUNT):
                if slot != replacement_slot and basis_valid[slot] != 0:
                    old_direction = basis[slot, particle]
                    old_matrix_direction = matrix_basis[slot, particle]
                    new_direction -= orthogonalization[slot, component] * old_direction
                    new_matrix_direction -= orthogonalization[slot, component] * old_matrix_direction
                    correction += projection[slot, component] * old_direction
                    matrix_correction += projection[slot, component] * old_matrix_direction
            if basis_denominator[replacement_slot, component] > 0.0:
                correction += projection[replacement_slot, component] * new_direction
                matrix_correction += projection[replacement_slot, component] * new_matrix_direction
            else:
                new_direction = wp.vec3(0.0)
                new_matrix_direction = wp.vec3(0.0)
            basis[replacement_slot, particle] = new_direction
...
@wp.kernel
def _measure_candidate_linear_residual(
    residual: wp.array[wp.vec3],
    inverse_scale: wp.array[wp.float32],
    packed_world: wp.array[wp.int32],
    packed_iterative: wp.array[wp.int32],
    world_active: wp.array[wp.int32],
    ...
    if packed_iterative[particle] != 0 and world_active[world] != 0:
        scaled = inverse_scale[particle] * residual[particle]
        value = wp.max(wp.abs(scaled[0]), wp.abs(scaled[1]))
        value = wp.max(value, wp.abs(scaled[2]))
        if not wp.isfinite(value):
            value = wp.float32(3.402823e38)
        wp.atomic_max(world_residual, world, value)


@wp.kernel
def _measure_candidate_linear_residual_and_advance_recycling(
    residual: wp.array[wp.vec3],
    inverse_scale: wp.array[wp.float32],
    packed_world: wp.array[wp.int32],
    packed_iterative: wp.array[wp.int32],
    ...
    if particle == 0:
        replacement_slot = replacement_cursor[0]
        basis_valid[replacement_slot] = recycling_started[0]
        recycling_started[0] = 1
        replacement_cursor[0] = (replacement_slot + 1) % _RECYCLED_CR_DIRECTION_COUNT
...
class DeformableFEMSystem:
    ...
    def __init__(
        ...
        if preconditioner not in ("incomplete_ldlt", "multilevel", "two_level", "block_jacobi"):
            raise ValueError(
                "LOX deformable preconditioner must be 'incomplete_ldlt', 'multilevel', "
                f"'two_level', or 'block_jacobi', got {preconditioner!r}."
            )
        if (
        ...
        if not isinstance(recycle_cr, bool):
            raise ValueError(f"LOX deformable CR recycling must be a boolean, got {recycle_cr}.")
        ...
        self.packed_component = wp.array(packed_component_np, dtype=wp.int32, device=self.device)
        self.packed_iterative = wp.array(packed_iterative_np, dtype=wp.int32, device=self.device)
        if self.recycle_cr and self.has_iterative_components:
            self.recycled_previous_velocity = wp.empty_like(self.smooth_velocity)
            self.recycled_previous_matrix_velocity = wp.empty_like(self.smooth_velocity)
            self.recycled_basis = wp.empty(
                (_RECYCLED_CR_DIRECTION_COUNT, self.particle_count),
                dtype=wp.vec3,
                device=self.device,
            )
            self.recycled_matrix_basis = wp.empty_like(self.recycled_basis)
            self.recycled_basis_denominator = wp.zeros(
                (_RECYCLED_CR_DIRECTION_COUNT, component_count),
                dtype=wp.float32,
                device=self.device,
            )
            self.recycled_basis_valid = wp.zeros(
                _RECYCLED_CR_DIRECTION_COUNT,
                dtype=wp.int32,
                device=self.device,
            )
            self.recycling_started = wp.zeros(1, dtype=wp.int32, device=self.device)
            self.recycled_replacement_cursor = wp.zeros(1, dtype=wp.int32, device=self.device)
            self.recycled_reductions = wp.zeros(
                (_RECYCLED_CR_REDUCTION_COUNT, component_count),
                dtype=wp.float32,
                device=self.device,
            )
            self.recycled_orthogonalization = wp.zeros(
                (_RECYCLED_CR_DIRECTION_COUNT, component_count),
                dtype=wp.float32,
                device=self.device,
            )
            ...
            self.recycled_basis = None
            self.recycled_matrix_basis = None
            self.recycled_basis_denominator = None
            self.recycled_basis_valid = None
            self.recycling_started = None
            self.recycled_replacement_cursor = None
        ...
        elif self.has_iterative_components and preconditioner in ("multilevel", "two_level"):
            preconditioner_type = DeformableMultilevel if preconditioner == "multilevel" else DeformableTwoLevel
        ...
        if not self.has_iterative_components:
            self.cr_state = None
        elif self.recycle_cr:
            assert self.recycled_previous_velocity is not None
            assert self.recycled_previous_matrix_velocity is not None
            assert self.recycled_basis is not None
            assert self.recycled_matrix_basis is not None
            assert self.recycled_basis_denominator is not None
            assert self.recycled_basis_valid is not None
            assert self.recycling_started is not None
            assert self.recycled_replacement_cursor is not None
            ...
            self.cr_state = _RecyclingCR(
                ...
                previous_velocity=self.recycled_previous_velocity,
                previous_matrix_velocity=self.recycled_previous_matrix_velocity,
                recycled_basis=self.recycled_basis,
                recycled_matrix_basis=self.recycled_matrix_basis,
                recycled_basis_denominator=self.recycled_basis_denominator,
                recycled_basis_valid=self.recycled_basis_valid,
                recycling_started=self.recycling_started,
                replacement_cursor=self.recycled_replacement_cursor,
        ...
        # Warp's stateful CR API does not expose its final recursive residual,
        # but retaining this allocated view avoids an otherwise redundant A*x.
        self.cr_residual = None if self.cr_state is None else self.cr_state._r_and_z_buf[0]
    ...
    def _finalize_consensus_system(self) -> None:
        ...
        if self.recycling_started is not None:
            self.recycling_started.zero_()
            self.recycled_basis_valid.zero_()
            self.recycled_basis_denominator.zero_()
            self.recycled_replacement_cursor.zero_()
        wp.launch(
            select_consensus_weight,
            dim=self.particle_count,
            inputs=[
    ...
    def solve_candidate(
        ...
        assert self.cr_residual is not None

        residual = self.cr_residual
        residual_inputs = [
            residual,
            self.full_inverse_weight,
            self.topology.packed_solve_world,
        ...
        if self.recycling_started is None:
            wp.launch(
                _measure_candidate_linear_residual,
                dim=self.particle_count,
                inputs=residual_inputs,
                outputs=[self.candidate_linear_residual],
                device=self.device,
            )
        else:
            assert self.recycled_replacement_cursor is not None
            assert self.recycled_basis_valid is not None
            wp.launch(
                _measure_candidate_linear_residual_and_advance_recycling,
                dim=self.particle_count,
                inputs=residual_inputs,
                outputs=[
                    self.candidate_linear_residual,
                    self.recycling_started,
                    self.recycled_replacement_cursor,
                    self.recycled_basis_valid,
                ],
                device=self.device,
            )
