...
__all__ = ["DeformableMultilevel", "DeformableTwoLevel"]
...
class DeformableTwoLevel:
    ...
    def __init__(
        ...
        Args:
            ...
            _global_coarse: Whether to add a third, component-global coarse correction.
        """
        ...
        self.global_coarse_block_count = 0
        self.global_coarse_slot_base = None
        self.global_coarse_slot_dimension = None
        self.global_coarse_group_offsets = None
        self.global_coarse_group_particles = None
        self.global_coarse_group_offset = None
        self.global_coarse_group_world = None
        self.particle_global_coarse_offset = None
        self.global_coarse_matrix = None
        self.global_coarse_right_hand_side = None
        self.global_coarse_solution = None
        self.global_coarse_solver = None
        if _global_coarse:
            component_coarse_block_counts = (
                aggregate_counts + _COARSE_BLOCK_AGGREGATE_COUNT - 1
            ) // _COARSE_BLOCK_AGGREGATE_COUNT
            component_coarse_block_offsets = np.concatenate(
                (
                    np.zeros(1, dtype=np.int32),
                    np.cumsum(component_coarse_block_counts, dtype=np.int32),
                )
            )
            coarse_block_count = int(component_coarse_block_offsets[-1])
            coarse_block_component = np.repeat(
                np.arange(component_coarse_block_counts.size, dtype=np.int32),
                component_coarse_block_counts,
            )
            coarse_block_local = np.arange(coarse_block_count, dtype=np.int32)
            coarse_block_local -= np.repeat(
                component_coarse_block_offsets[:-1],
                component_coarse_block_counts,
            )

            global_info = DenseSquareMultiLinearInfo()
            global_info.finalize(
                dimensions=(3 * component_coarse_block_counts).tolist(),
                dtype=wp.float32,
                itype=wp.int32,
                device=self.device,
            )
            global_matrix_offsets = np.asarray(global_info.mio.numpy(), dtype=np.int64)
            global_vector_offsets = np.asarray(global_info.vio.numpy(), dtype=np.int64)

            particle_global_coarse_offset = np.full(row_count, -1, dtype=np.int32)
            particle_coarse_blocks = aggregate_block[particle_aggregates]
            particle_global_components = coarse_block_component[particle_coarse_blocks]
            particle_global_coarse_offset[iterative_rows] = (
                global_vector_offsets[particle_global_components] + 3 * coarse_block_local[particle_coarse_blocks]
            )

            global_group_particle_offsets = aggregate_offsets[coarse_block_offsets]
            global_group_first_particles = aggregate_particles[global_group_particle_offsets[:-1]]
            global_group_coarse_offset = particle_global_coarse_offset[global_group_first_particles]
            global_group_world = packed_world.numpy().astype(np.int32, copy=False)[global_group_first_particles]

            global_coarse_slot_base = np.full(columns.shape[0], -1, dtype=np.int32)
            global_coarse_slot_dimension = np.zeros(columns.shape[0], dtype=np.int32)
            valid_row_blocks = aggregate_block[valid_row_aggregates]
            valid_column_blocks = aggregate_block[valid_column_aggregates]
            valid_components = coarse_block_component[valid_row_blocks]
            valid_dimensions = 3 * component_coarse_block_counts[valid_components]
            global_coarse_slot_base[valid_slots] = (
                global_matrix_offsets[valid_components]
                + 3 * coarse_block_local[valid_row_blocks] * valid_dimensions
                + 3 * coarse_block_local[valid_column_blocks]
            )
            global_coarse_slot_dimension[valid_slots] = valid_dimensions

            self.global_coarse_block_count = coarse_block_count
            self.global_coarse_slot_base = wp.array(
                global_coarse_slot_base,
                dtype=wp.int32,
                device=self.device,
            )
            self.global_coarse_slot_dimension = wp.array(
                global_coarse_slot_dimension,
                dtype=wp.int32,
                device=self.device,
            )
            self.global_coarse_group_offsets = wp.array(
                global_group_particle_offsets,
                dtype=wp.int32,
                device=self.device,
            )
            self.global_coarse_group_particles = self.aggregate_particles
            self.global_coarse_group_offset = wp.array(
                global_group_coarse_offset,
                dtype=wp.int32,
                device=self.device,
            )
            self.global_coarse_group_world = wp.array(
                global_group_world,
                dtype=wp.int32,
                device=self.device,
            )
            self.particle_global_coarse_offset = wp.array(
                particle_global_coarse_offset,
                dtype=wp.int32,
                device=self.device,
            )
            self.global_coarse_matrix = wp.zeros(
                global_info.total_mat_size,
                dtype=wp.float32,
                device=self.device,
            )
            self.global_coarse_right_hand_side = wp.zeros(
                global_info.total_vec_size,
                dtype=wp.float32,
                device=self.device,
            )
            self.global_coarse_solution = wp.zeros(
                global_info.total_vec_size,
                dtype=wp.float32,
                device=self.device,
            )
            global_coarse_operator = DenseLinearOperatorData(
                info=global_info,
                mat=self.global_coarse_matrix,
            )
            self.global_coarse_solver = LLTBlockedSolver(
                operator=global_coarse_operator,
                factorize_block_size=24,
                solve_block_size=24,
                solve_block_dim=256,
                dtype=wp.float32,
    ...
    def factorize(self) -> None:
        ...
        if self.global_coarse_solver is not None:
            self.global_coarse_matrix.zero_()
            wp.launch(
                _assemble_coarse_matrix,
                dim=self.system_matrix.values.shape[0],
                inputs=[
                    self.system_matrix.values,
                    self.global_coarse_slot_base,
                    self.global_coarse_slot_dimension,
                ],
                outputs=[self.global_coarse_matrix],
                device=self.device,
            )
            self.global_coarse_solver.compute(self.global_coarse_matrix)
    ...
    def _matvec(
        ...
        if self.global_coarse_solver is not None:
            if self.device.is_cuda:
                wp.launch(
                    _restrict_residual_by_aggregate,
                    dim=self.global_coarse_block_count * _AGGREGATE_BLOCK_DIM,
                    inputs=[
                        x,
                        self.global_coarse_group_offsets,
                        self.global_coarse_group_particles,
                        self.global_coarse_group_offset,
                        self.global_coarse_group_world,
                        self.world_active,
                        self.world_status,
                    ],
                    outputs=[self.global_coarse_right_hand_side],
                    block_dim=_AGGREGATE_BLOCK_DIM,
                    device=self.device,
                )
            else:
                wp.launch(
                    _restrict_residual_by_aggregate_cpu,
                    dim=self.global_coarse_block_count,
                    inputs=[
                        x,
                        self.global_coarse_group_offsets,
                        self.global_coarse_group_particles,
            ...
            self.global_coarse_solver.solve(
                self.global_coarse_right_hand_side,
                self.global_coarse_solution,
            )
            wp.launch(
                _add_coarse_correction,
                dim=self.row_count,
                inputs=[
                    self.global_coarse_solution,
                    self.particle_global_coarse_offset,
                    self.packed_world,
                    self.world_active,
                    self.world_status,
                    alpha,
...
class DeformableMultilevel(DeformableTwoLevel):
    """Apply fine, block-local coarse, and component-global coarse corrections."""
