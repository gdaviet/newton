...
from ...core.materials import MaterialMixMode
...
if TYPE_CHECKING:
    ...
    from ....config import MaterialManagerConfig
...
"""The contact has no finite positive scalar Delassus coefficient and is not projected."""
...
_MATERIAL_MIX_MODE_MULTIPLY = int(MaterialMixMode.MULTIPLY)
_MATERIAL_MIX_MODE_MAX = int(MaterialMixMode.MAX)
_MATERIAL_MIX_MODE_MIN = int(MaterialMixMode.MIN)
...
@wp.func
def _mix_contact_material(value1: float, value2: float, mixmode: int) -> float:
    if mixmode == _MATERIAL_MIX_MODE_MULTIPLY:
        return value1 * value2
    if mixmode == _MATERIAL_MIX_MODE_MAX:
        return wp.max(value1, value2)
    if mixmode == _MATERIAL_MIX_MODE_MIN:
...
def _reject_contact(
    ...
    # A malformed source record can be discarded locally. Retain its status for
    # diagnostics without suppressing valid contacts or failing the LOX world.
    if status == DEFORMABLE_CONTACT_STATUS_MALFORMED:
        return
...
def _adapt_soft_contacts(
    ...
    soft_friction: float,
    soft_restitution: float,
    friction_mix_mode: int,
    restitution_mix_mode: int,
    particle_indices: wp.array2d[wp.int32],
    ...
    if (
        ...
        or shape >= shape_friction.shape[0]
        or shape >= shape_restitution.shape[0]
    ...
    if (
        ...
        or not wp.isfinite(shape_friction[shape])
        or shape_friction[shape] < 0.0
        or not wp.isfinite(shape_restitution[shape])
        or shape_restitution[shape] < 0.0
    ):
        _reject_contact(
            contact,
            feature.world,
    ...
    friction = _mix_contact_material(soft_friction, shape_friction[shape], friction_mix_mode)
    restitution = _mix_contact_material(
        soft_restitution,
        shape_restitution[shape],
        restitution_mix_mode,
    )
    if not wp.isfinite(friction) or friction < 0.0 or not wp.isfinite(restitution) or restitution < 0.0:
        _reject_contact(
            contact,
            feature.world,
            DEFORMABLE_CONTACT_STATUS_MALFORMED,
...
def _finalize_contact_delassus(
    ...
    if value == 0.0 and contact_body[contact] < 0:
        # Contacts between prescribed deformable features have no response to
        # solve for. Retain the per-contact diagnostic, but discard the contact
        # locally instead of failing an otherwise valid world.
        if world >= 0:
...
class DeformableContactSystem:
    ...
    def __init__(
        ...
        material_config: MaterialManagerConfig | None = None,
    ):
        ...
        Args:
            ...
            material_config: Kamino material property mixing configuration.
        """
    ...
    def _validate_source_arrays(
        ...
        if self.rigid_contact_capacity > 0:
            arrays["shape_material_mu"] = self.model.shape_material_mu
            arrays["shape_material_restitution"] = self.model.shape_material_restitution
    ...
    def prepare(
        ...
        soft_friction = float(self.model.soft_contact_mu)
        soft_restitution = float(self.model.soft_contact_restitution)
        if not np.isfinite(soft_friction) or soft_friction < 0.0:
            raise ValueError("LOX deformable contact friction must be finite and non-negative.")
        if not np.isfinite(soft_restitution) or soft_restitution < 0.0:
            raise ValueError("LOX deformable contact restitution must be finite and non-negative.")
        friction_mix_mode = MaterialMixMode.AVERAGE
        restitution_mix_mode = MaterialMixMode.MIN
        if self.material_config is not None:
            friction_mix_mode = MaterialMixMode.from_string(self.material_config.friction_mix_mode)
            restitution_mix_mode = MaterialMixMode.from_string(self.material_config.restitution_mix_mode)
        ...
        if contacts is not None and self.rigid_contact_capacity > 0:
            wp.launch(
                ...
                inputs=[
                    ...
                    soft_friction,
                    soft_restitution,
                    int(friction_mix_mode),
                    int(restitution_mix_mode),
                ],
                outputs=[
