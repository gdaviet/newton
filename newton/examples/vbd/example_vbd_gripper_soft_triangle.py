...
###########################################################################
# Example VBD Gripper — Soft Triangle (water-tight FACE contact)
#
# A parallel-jaw gripper with CUBE finger pads grips a single soft triangle
# at its CENTER, then lifts and holds it in the air.
#
# The triangle's three vertices sit well outside the gripper's reach, so the
...
PARAMS = {
    ...
    "sim_substeps": 20,
    "solver_iterations": 5,
    # LOX needs more outer coupling iterations than VBD for the loaded joints.
    "lox_solver_iterations": 20,
    "gravity": (0.0, 0.0, -9.8),
    "num_frames": 360,
    # water-tight rigid-soft contact. True = grip the triangle by its face (this feature);
    # False = legacy per-particle contact only (== main behavior), where the triangle slips.
    ...
    # gripper gap (half-gap per finger). The vertical triangle is a zero-thickness sheet at
    # y=0, so (unlike a sheet whose edges start inside open jaws) it would free-fall before
    # slow jaws could reach it. The jaws therefore START already gently holding it (inner
    # faces ~3 mm inside the radius shell) and firm up slightly -- a gentle, no-fall grip.
    "open_half_gap": 0.037,
    "closed_half_gap": 0.036,
    ...
    # soft triangle standing VERTICALLY in the x-z plane (face normal along y). Big, with
    # its centroid on the grip point; vertices far out in x and z (outside the jaws). The
    # near-zero y-thickness < open_half_gap so the OPEN jaws start clear and sweep in.
    # tri_up = 2*tri_down keeps the centroid on the grip point.
    "tri_half_x": 0.18,
    "tri_down": 0.12,
    "tri_up": 0.24,
    # apex shifted off the grip's x=0 line (|x| > finger_half) so NO vertex can fall into the
    # jaws -- with water-tight off the triangle then drops cleanly instead of snagging by a tip.
    "tri_apex_x": 0.07,
    "tri_mass": 0.1,
    "particle_radius": 0.01,
    "tri_color": (0.2, 0.8, 0.2),
...
class Example:
    def __init__(self, viewer, args):
        ...
        if self.solver_type == "lox":
            ...
            config.lox.max_iterations = self.params["lox_solver_iterations"]
    ...
    def _add_soft_mesh(self, builder):
        """A single soft triangle standing VERTICALLY in the x-z plane (its face normal
        along y, the gripper's closing axis), big, with the grip point (0, 0, z) on its
        face. All three vertices sit outside the jaws (base corners far in x, apex shifted
        off the x=0 line and high in z), so only the central face crosses the pinch — only
        the water-tight FACE pass can grip it."""
        p = self.params
        z = p["grab_z"] - p["finger_z_offset"]
        hx, up, dn = p["tri_half_x"], p["tri_up"], p["tri_down"]
        verts = [
            wp.vec3(-hx, 0.0, z - dn),
            wp.vec3(hx, 0.0, z - dn),
            wp.vec3(p["tri_apex_x"], 0.0, z + up),
        ]
        m = p["tri_mass"] / 3.0
        rad = p["particle_radius"]
        i0 = builder.add_particle(verts[0], wp.vec3(0.0), m, radius=rad)
        i1 = builder.add_particle(verts[1], wp.vec3(0.0), m, radius=rad)
        i2 = builder.add_particle(verts[2], wp.vec3(0.0), m, radius=rad)
        builder.add_triangle(i0, i1, i2)
        self._mesh_particles = (i0, i1, i2)
    ...
    def _build_waypoints(self):
        ...
        # (target gantry pos, duration, gripper_frac)
        self._waypoints = [
    ...
    def test_final(self):
        """The gripped triangle must be held in the air, not dropped to the ground."""
        q = self.state_0.particle_q.numpy()
        mean_z = float(q[list(self._mesh_particles), 2].mean())
        if not mean_z > 0.3:
            raise AssertionError(f"soft triangle was dropped (mean z={mean_z:.3f}); water-tight grip failed")
