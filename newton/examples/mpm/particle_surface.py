...
@wp.kernel
def _accumulate_vertex_normals(
    ...
    p1 = vertices[i1]
    p2 = vertices[i2]
    face_normal = wp.cross(p1 - p0, p2 - p0)
...
@wp.kernel
def _normalize_vertex_normals(normals: wp.array[wp.vec3]):
    i = wp.tid()
    n = normals[i]
    length = wp.length(n)
    normals[i] = wp.where(length > 1.0e-12, n / length, wp.vec3(0.0, 0.0, 1.0))


def compute_vertex_normals(vertices: wp.array[wp.vec3], indices: wp.array[wp.int32]) -> wp.array[wp.vec3]:
    """Compute area-weighted vertex normals for a triangle mesh."""
    normals = wp.zeros(vertices.shape[0], dtype=wp.vec3, device=vertices.device)
    wp.launch(
        _accumulate_vertex_normals,
        dim=indices.shape[0] // 3,
        inputs=[vertices, indices, normals],
        device=vertices.device,
    )
    wp.launch(_normalize_vertex_normals, dim=vertices.shape[0], inputs=[normals], device=vertices.device)
    return normals
