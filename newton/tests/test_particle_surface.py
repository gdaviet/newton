...
from newton.examples.mpm.particle_surface import ParticleSurface, _redistance_step, extract_particle_surface
...
def test_anisotropic(test, device):
    ...
    verts, _indices, _normals = ctx.extract(positions, radii=radii)
    test.assertIsNotNone(verts, "Anisotropic extraction produced no surface")
    test.assertGreater(verts.shape[0], 0)

    # Field is converted to an SDF, so it should straddle the zero surface.
    field_np = ctx.field.numpy()
    test.assertLess(field_np.min(), 0.0, "Anisotropic SDF should be negative inside")
    test.assertGreater(field_np.max(), 0.0, "Anisotropic SDF should be positive outside")
...
def test_redistance_via_api(test, device):
    ...
    verts0, _indices0, _ = ctx_no_redist.extract(positions, radii=radii)
    verts1, _indices1, _ = ctx_redist.extract(positions, radii=radii)
