...
- Add opt-in ASPG to `SolverImplicitMPM.Config(solver="aspg")` with BB2 spectral step adaptation for coupled material and collider projection.
...
- Add opt-in APGD to `SolverImplicitMPM.Config(solver="apgd")` for coupled material and collider projection, including non-associated flow, viscosity, and independent per-world convergence.
