...
- Change experimental `CouplingInterface` hooks to concrete mixin methods with default behavior. Remove `CouplingInterface.Hook` / `coupling_unsupported`; override the hook method and raise `NotImplementedError` for unsupported operations. Proxy harvest overrides must accept the new `body_qd_before` or `particle_qd_before` keyword arguments.
